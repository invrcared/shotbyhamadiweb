...
export default async function AlbumView({ params }: { params: Promise<{ slug: string }> }) {
    try {
        ...
        return (
            <div className="min-h-screen bg-[#000000] text-white pt-24 px-4 md:px-10 pb-24 selection:bg-[#A1A1AA] selection:text-black font-sans">
                ...
                <header className="mb-16 md:mb-24 max-w-4xl">
                    <h1 className="text-3xl md:text-5xl lg:text-7xl font-light tracking-wide mb-6">{album.title || "Untitled Album"}</h1>
        ...
        return (
            <div className="min-h-screen bg-black flex items-center justify-center text-red-500 p-8">
                <p>Album View Error: {error.message}</p>
            </div>
        );
    }
}
